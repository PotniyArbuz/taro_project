...
async function getYandexGPTResponse(question, selectedCards, userId) {
    try {
        const response = await fetch('https://ai-girls.ru/yandex-gpt', {
            ...
            body: JSON.stringify({ question, cards: selectedCards, user_id: userId, chat_id: chatId, source: "premium7" })
        });
...
async function startReading() {
    ...
    const question = `Информация об интересующем человеке:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo}` + "\nРасклад “Что между нами?” (7 карт).\nКарта 1 — Как он(а) меня видит?\nКарта 2 — Как он(а) меня чувствует?\nКарта 3 — Как он(а) ведёт себя на самом деле?\nКарта 4 — Как он(а) воспринимает наши отношения?\nКарта 5 — Чего он(а) хочет от меня?\nКарта 6 — Что мешает нашим отношениям??\nКарта 7 — Возможное будущее. Ответ должен быть максимально развернутым. Количество слов в ответе должно быть не менее 1000.";
