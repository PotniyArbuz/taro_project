// --------------------- NEW Spread: Феникс возрождения (8 карт, 3-2-3) ---------------------
...
async function startReading() {
    ...
    const question =
        `Расклад \"Феникс возрождения\" (8 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Прошлый кризис – Что вызвало боль или трудности?\n` +
        `2. Текущее состояние – Где вы находитесь сейчас?\n` +
        `3. Остатки боли – Что ещё нужно исцелить?\n` +
        `4. Скрытая сила – Какой ресурс поможет возродиться?\n` +
        `5. Шаг к исцелению – Что сделать для восстановления?\n` +
        `6. Поддержка Вселенной – Как мир помогает вам?\n` +
        `7. Новая энергия – Что принесёт возрождение?\n` +
        `8. Будущее сияние – К чему приведёт трансформация?\n\n` +
    ...
    // Геометрия раскладки (3-2-3, крылья феникса)
    ...
    const totalCenterRowWidth = baseCardWidth * 2 + gap;
    const startLeftRow = Math.max((screenWidth - totalRowWidth) / 2, 10);
    const startLeftCenter = Math.max((screenWidth - totalCenterRowWidth) / 2, 10);
    ...
    selectedCards.forEach((card, index) => {
        let row, col;
        if (index < 3) { // Первая строка: 3 карты (левое крыло, прошлое)
            row = 0;
            col = index;
        } else if (index < 5) { // Вторая строка: 2 карты (центр, трансформация)
            row = 1;
            col = index - 3;
        } else { // Третья строка: 3 карты (правое крыло, будущее)
            row = 2;
            col = index - 5;
        }
        ...
        const leftPos = row === 1 ? startLeftCenter + col * cardWidth : startLeftRow + col * cardWidth;
        ...
        // Подсветка центральных карт (трансформация, позиции 4 и 5)
        if (index === 3 || index === 4) {
            cardElem.style.boxShadow = "0 0 15px 5px rgba(255, 69, 0, 0.7)"; // Оранжево-красное свечение для феникса
        }
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем ваш путь возрождения...",
            "Формируем шаги к исцелению...",
            "Раскрываем пламя новой энергии..."
        ];
        let i = 0;
