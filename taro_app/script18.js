// --------------------- Spread: Энергия жизни (8 карт, 2-3-3) ---------------------
...
async function startReading() {
    ...
    const question =
        `Расклад \"Энергия жизни\" (8 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Текущая энергия – Каков ваш уровень энергии?\n` +
        `2. Источник истощения – Что забирает силы?\n` +
        `3. Источник силы – Что питает вашу энергию?\n` +
        `4. Внутренний блок – Какой ментальный барьер мешает?\n` +
        `5. Внешняя поддержка – Кто или что восстанавливает силы?\n` +
        `6. Действие для восстановления – Как вернуть энергию?\n` +
        `7. Будущий потенциал – Какой уровень энергии возможен?\n` +
        `8. Совет света – Финальное напутствие.\n\n` +
    ...
    // Геометрия раскладки (2-3-3, солнце)
    ...
    const totalFirstRowWidth = baseCardWidth * 2 + gap;
    const startLeftRow = Math.max((screenWidth - totalRowWidth) / 2, 10);
    const startLeftFirstRow = Math.max((screenWidth - totalFirstRowWidth) / 2, 10);
    ...
    selectedCards.forEach((card, index) => {
        let row, col;
        if (index < 2) { // Первая строка: 2 карты (верхние лучи)
            row = 0;
            col = index;
        } else if (index < 5) { // Вторая строка: 3 карты (центр, включая кверента)
            row = 1;
            col = index - 2;
        } else { // Третья строка: 3 карты (нижние лучи)
            row = 2;
            col = index - 5;
        }
        ...
        const leftPos = row === 0 ? startLeftFirstRow + col * cardWidth : startLeftRow + col * cardWidth;
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем вашу энергию...",
            "Формируем путь к восстановлению...",
            "Раскрываем источник света..."
        ];
        let i = 0;
