// --------------------- Spread: Искры вдохновения (9 карт, 3-3-3) ---------------------
...
async function startReading() {
    ...
    const question =
        `Расклад \"Искры вдохновения\" (9 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Текущая искра – Что вдохновляет вас сейчас?\n` +
        `2. Прошлый источник – Что вдохновляло раньше?\n` +
        `3. Текущий застой – Что блокирует вдохновение?\n` +
        `4. Внутренний ресурс – Какой внутренний потенциал использовать?\n` +
        `5. Внешний стимул – Что во внешнем мире вдохновляет?\n` +
        `6. Действие для пробуждения – Как разжечь искру?\n` +
        `7. Поддержка окружения – Кто или что помогает?\n` +
        `8. Краткосрочный результат – Что даст вдохновение?\n` +
        `9. Долгосрочная мечта – Куда приведут новые идеи?\n\n` +
    ...
    // Геометрия раскладки (3-3-3, звезда)
    ...
    const startLeftRow = Math.max((screenWidth -
 ...
 totalRowWidth) / 2, 10);
    ...
    selectedCards.forEach((card, index) => {
        ...
        // Подсветка центральной карты (кверент, позиция 1) и лёгкий поворот для лучей звезды
        if (index === 0) {
            cardElem.style.boxShadow = "0 0 15px 5px rgba(255, 223, 0, 0.7)"; // Золотое свечение для центра звезды
        } else {
            // Лёгкий поворот для имитации лучей звезды
            const angles = [-15, 15, 0, -10, 10, -5, 5, 0, 0]; // Углы для лучей
            cardElem.style.transform = `rotate(${angles[index]}deg)`;
        }
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем ваши искры вдохновения...",
            "Формируем путь к новым идеям...",
            "Раскрываем звёзды вашего потенциала..."
        ];
        let i = 0;
