// Tarot Spread "Лестница карьеры" – 9 cards (3x3 grid)
// Theme: Career growth, professional goals, and overcoming obstacles
// Layout: 3x3 grid, symbolizing past (bottom), present (middle), and future (top)
...
async function startReading() {
    ...
    const question =
        `Расклад "Лестница карьеры" (9 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Прошлое в карьере – Что сформировало текущую профессиональную ситуацию?\n` +
        `2. Текущая позиция – Где кверент находится сейчас в профессиональном плане?\n` +
        `3. Скрытый потенциал – Неиспользованные возможности или таланты.\n` +
        `4. Препятствие на пути – Что мешает карьерному росту?\n` +
        `5. Ключевая возможность – Какой шанс стоит использовать?\n` +
        `6. Необходимое действие – Конкретный шаг для продвижения вперёд.\n` +
        `7. Поддержка окружения – Кто или что поможет в карьере?\n` +
        `8. Краткосрочный результат – Что произойдёт в ближайшие месяцы?\n` +
        `9. Долгосрочная перспектива – Куда приведёт текущий путь?\n\n` +
        `Ответ должен быть максимально развернутым, с акцентом на карьерные и профессиональные аспекты. Количество слов в ответе не менее 1200.`;
    ...
    const rowTop = [
        deckTop + deckHeight + deckHeight / 2, // Bottom row (past)
        deckTop + deckHeight + deckHeight / 2 + deckHeight + gap, // Middle row (present)
        deckTop + deckHeight + deckHeight / 2 + (deckHeight + gap) * 2 // Top row (future)
    ];
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем карьерные перспективы...",
            "Формируем прорицание...",
            "Раскрываем профессиональные возможности..."
        ];
        let i = 0;
