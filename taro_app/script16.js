// --------------------- NEW Spread: Преодоление теней (8 карт, 3-2-3) ---------------------
...
async function startReading() {
    ...
    const question =
        `Расклад \"Преодоление теней\" (8 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Главный страх – Какой страх доминирует сейчас?\n` +
        `2. Источник страха – Откуда он взялся?\n` +
        `3. Влияние страха – Как он ограничивает жизнь?\n` +
        `4. Скрытая сила – Что поможет преодолеть страх?\n` +
        `5. Первый шаг – Как начать борьбу со страхом?\n` +
        `6. Поддержка окружения – Кто или что поможет?\n` +
        `7. Результат преодоления – Что ждёт на другой стороне?\n` +
        `8. Совет Вселенной – Финальное напутствие.\n\n` +
    ...
    // Геометрия раскладки (3-2-3, мост через реку страхов)
    ...
    selectedCards.forEach((card, index) => {
        let row, col;
        if (index < 3) { // Первая строка: 3 карты (слева, тени)
            row = 0;
            col = index;
        } else if (index < 5) { // Вторая строка: 2 карты (центр, переход)
            row = 1;
            col = index - 3;
        } else { // Третья строка: 3 карты (справа, свет)
            row = 2;
            col = index - 5;
        }
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем страхи и возможности...",
            "Формируем путь к преодолению...",
            "Раскрываем свет на другой стороне..."
        ];
        let i = 0;
