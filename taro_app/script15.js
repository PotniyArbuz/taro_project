// Tarot Spread "Река творчества" – 9 cards (3x3 grid)
// Theme: Developing creative potential and overcoming creative blocks
// Layout: 3x3 grid, representing a river's flow (source, current, estuary)
...
async function startReading() {
    ...
    const question =
        `Расклад "Река творчества" (9 карт)\n\n` +
        `Информация о кверенте:\nИмя: ${name}\nВозраст: ${age}\nДополнительная информация: ${additionalInfo || "—"}\n\n` +
        `Позиции карт:\n` +
        `1. Творческий потенциал – Что лежит в основе вашего творчества?\n` +
        `2. Прошлый опыт – Как вы выражали творчество ранее?\n` +
        `3. Текущий блок – Что мешает творческому потоку?\n` +
        `4. Внутренний ресурс – Какой талант можно использовать?\n` +
        `5. Внешний стимул – Что вдохновляет извне?\n` +
        `6. Действие для пробуждения – Как возродить творчество?\n` +
        `7. Краткосрочный результат – Что даст первый шаг?\n` +
        `8. Долгосрочный потенциал – Куда приведёт творчество?\n` +
        `9. Совет Музы – Финальное вдохновение от карт.\n\n` +
        `Ответ должен быть максимально развернутым, с акцентом на развитие творческого потенциала и преодоление блоков. Количество слов в ответе не менее 1200.`;
    ...
    const rowTop = [
        deckTop + deckHeight + deckHeight / 2, // Bottom row (source)
        deckTop + deckHeight + deckHeight / 2 + deckHeight + gap, // Middle row (current)
        deckTop + deckHeight + deckHeight / 2 + (deckHeight + gap) * 2 // Top row (estuary)
    ];
    ...
    setTimeout(() => {
        ...
        const messages = [
            ...
            "Анализируем творческий потенциал...",
            "Формируем прорицание...",
            "Раскрываем пути к вдохновению..."
        ];
        let i = 0;
