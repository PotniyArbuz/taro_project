...
async function getYandexGPTResponse(question, selectedCards, userId) {
    try {
        const response = await fetch('https://ai-girls.ru/yandex-gpt', {
            ...
            body: JSON.stringify({ question, cards: selectedCards, user_id: userId, chat_id: chatId, source: "daily" })
        });
...
async function startReading() {
    ...
    "Расклад “Ежедневное руководство” (7 карт)\n" +
    "1 карта - карта в этой позиции отражает энергетическую атмосферу в первые часы дня.\n" +
    "2 карта - здесь показывается самая приоритетная цель на сегодня.\n" +
    "3 карта - карта-флажок, сигнализирующий о том, на что нужно обратить внимание, чтобы избежать ошибок.\n" +
    "4 карта - ваша поддержка – внутренний или внешний потенциал, на который стоит опереться.\n" +
    "5 карта - карта здесь предвещает событие, которое придёт внезапно и изменит ход дня.\n" +
    "6 карта - чему день вас научит и какую мудрость вы унесёте с собой.\n" +
    "7 карта - итог и главный совет на завтра." +
